...
const authMiddleware = (req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  // Use robust defaults if env vars are missing or empty
  const targetUser = (process.env.APP_USERNAME || 'Harsh').trim() || 'Harsh';
  const targetPass = (process.env.APP_PASSWORD || '1234').trim() || '1234';
  const expectedToken = Buffer.from(`${targetUser}:${targetPass}`).toString('base64');
...
app.post('/api/login', (req, res) => {
  ...
  // Case-insensitive username and trimmed password for better UX
  if (username?.trim().toLowerCase() === targetUser.toLowerCase() && password?.trim() === targetPass) {
    const token = Buffer.from(`${targetUser}:${targetPass}`).toString('base64');
    res.json({ token });
  } else {
